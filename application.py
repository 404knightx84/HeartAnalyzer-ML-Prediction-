...
async def api_predict(request: PredictionRequest):
    ...
    return {
        "prediction": result,
        "confidence": heart_disease_prob,
        "risk_probability": risk_prob,
        "input_data": {
            "age": request.age,
            "sex": "Male" if request.sex == 1 else "Female",
            "chest_pain_type": request.cp,
            "resting_bp": request.trestbps,
            "cholesterol": request.chol,
            "fasting_bs": request.fbs,
            "resting_ecg": request.restecg,
            "max_heart_rate": request.thalach,
            "exercise_angina": "Yes" if request.exang == 1 else "No",
            "st_depression": request.oldpeak,
            "st_slope": request.slope,
            "major_vessels": request.ca,
            "thalassemia": request.thal
        }
    }
